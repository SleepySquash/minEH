...
#include <unistd.h>
        ...
        std::string getPathWOResourcePath(const std::string& path)
        {
            ...
            if (respath.length() == 0 || respath == "" || !strcmp(respath.c_str(), path.c_str())) return path;
    ...
    bool atob(const std::wstring& stringValue) {
        return (stringValue == L"true" || stringValue == L"True" || stringValue == L"T" || stringValue == L"истина" || stringValue == L"1"); }
